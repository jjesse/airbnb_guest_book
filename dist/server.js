...
const normalizeToUtcDate = (date) => (new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate())));
const getStayDuration = (entry) => {
...
const buildOccupancyByMonth = (entries) => {
    ...
    entries.forEach((entry) => {
        ...
        while (currentMonth < checkOut) {
            ...
            const overlapDays = Math.max(0, Math.ceil((overlapEnd.getTime() - overlapStart.getTime()) / MS_PER_DAY));
            if (overlapDays > 0) {
...
app.get('/api/entries/export', authMiddleware, async (req, res, next) => {
    try {
...
app.get('/api/analytics/statistics', authMiddleware, async (req, res, next) => {
    try {
        const entries = await Entry.find().sort('-date');
